...
import { ArrowLeftIcon, ArrowRightIcon, LocationPinIcon, UpvoteIcon, BookmarkIcon } from './icons';
             ...
             <div className="flex items-center gap-1">
                <button
                    ...
                    className={`p-2 rounded-full transition-colors ${
                        isSaved ? 'text-yellow-400' : 'text-[--text-secondary]'
                    } hover:text-yellow-400 hover:bg-yellow-400/10`}
                    aria-label={isSaved ? 'Unsave post' : 'Save post'}
                >
                    <BookmarkIcon className="w-5 h-5" isFilled={isSaved} />
                </button>
                <button onClick={handleUpvote} className="flex items-center gap-1.5 text-sm font-semibold text-[--text-secondary] hover:text-[--primary-color] transition-colors rounded-full px-3 py-1 bg-white/5 hover:bg-[--primary-color]/10">
