                        ...
                        <div className={`${post.images && post.images.length > 0 ? 'md:w-1/2' : 'w-full'} p-6 space-y-6`}>
                            ...
                            <div>
                                <div className="flex items-start justify-between gap-4 mb-2">
                                    ...
                                    <div className="bg-emerald-500/10 text-emerald-400 text-xs font-bold px-3 py-1 rounded-lg border border-emerald-500/20 whitespace-nowrap">
                                        {post.category}
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                                    ...
                                    <p className="text-2xl font-bold text-emerald-400">{formatPrice(post.price)}</p>
                                </div>
                                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Area</p>
                                    <p className="text-2xl font-bold text-white">{post.squareFeet.toLocaleString()} <span className="text-sm text-gray-400">sq.ft</span></p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 text-white">
                                    <LocationPinIcon className="w-5 h-5 text-emerald-500" />
                                    ...
                                    <div className="space-y-2">
                                        {post.phone && (
                                            <div className="flex items-center gap-2 text-white">
                                                ...
                                                <a href={`tel:${post.phone}`} className="hover:text-emerald-400 transition-colors">{post.phone}</a>
                                            </div>
                                        )}
                                        {post.email && (
                                            <div className="flex items-center gap-2 text-white">
                                                <EnvelopeIcon className="w-5 h-5 text-blue-500" />
                                                <a href={`mailto:${post.email}`} className="hover:text-emerald-400 transition-colors">{post.email}</a>
                                            </div>
                                        )}
                                    </div>
                                </div>
